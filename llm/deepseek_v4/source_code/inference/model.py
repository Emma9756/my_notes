...
class Compressor(nn.Module):
    ...
    def __init__(self, args: ModelArgs, compress_ratio: int = 4, head_dim: int = 512, rotate: bool = False):
        ...
        # 权重和偏差系数，coff可以理解为头的数量，CSA 2个头， HCA 1个头
    ...
    def forward(self, x: torch.Tensor, start_pos: int):
        ...
        if start_pos == 0:
            ...
            self.kv_state[:bsz, :4] = kv[:, 100-4 : 100]
            self.score_state[:bsz, :4] = score[:, 100-4 : 100] + self.ape

            kv, self.kv_state[:bsz, 4 : 4+3] = kv.split([100, 3], dim=1)
            self.score_state[:bsz, 4 : 4+3] = score[:, 100:] + self.ape[:3]
            score = score[:, :100]
            '''
            ...
            # 假设seqlen=103，这里存储倒数第一组的4条 [96:100]
            if overlap and cutoff >= ratio:
                self.kv_state[:bsz, :ratio] = kv[:, cutoff-ratio : cutoff]
                self.score_state[:bsz, :ratio] = score[:, cutoff-ratio : cutoff] + self.ape
            # 存储尾巴 [100:103]，同时kv = kv[0:100], score = score[0:100]，用于进行压缩
            if remainder > 0:
...
class Block(nn.Module):
    ...
    def hc_pre(self, x: torch.Tensor, hc_fn: torch.Tensor, hc_scale: torch.Tensor, hc_base: torch.Tensor):
        ...
        x = x.flatten(2).float() # x: [b,s,hc*d]
        rsqrt = torch.rsqrt(x.square().mean(-1, keepdim=True) + self.norm_eps)
        mixes = F.linear(x, hc_fn) * rsqrt # x: [b,s,hc*d] @ hc_fn: [mix_hc,hc*d] = mixes[b,s,mix_hc] mix_hc = (2 + hc_mult) * hc_mult
        pre, post, comb = hc_split_sinkhorn(mixes, hc_scale, hc_base, self.hc_mult, self.hc_sinkhorn_iters, self.hc_eps)
        y = torch.sum(pre.unsqueeze(-1) * x.view(shape), dim=2) # pre.unsqueeze(-1): [B, S, 4, 1] x: [b,s,hc,d] = [b,s,hc,d]  sum [b,s,d] 
    ...
    def hc_post(self, x: torch.Tensor, residual: torch.Tensor, post: torch.Tensor, comb: torch.Tensor):
        ...
        # post.unsqueeze(-1) * x.unsqueeze(-2) [b, s, hc, d]

        # comb.unsqueeze(-1):     [b, s, hc, hc, 1]
        # residual.unsqueeze(-2): [b, s, hc, 1,  d]
        # * [b, s, hc, hc, d] 理解 [b, s, source_hc, target_hc, d]
        # [b, s, hc, hc, d] --sum(dim=2)--> [b, s, hc, d]
        return y.type_as(x)
